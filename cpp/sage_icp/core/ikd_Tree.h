...
#define Q_LEN_init 5000

using namespace std;
namespace sage_icp {
struct semicp_PointType
{
    float x,y,z;
    int label;
    semicp_PointType (float px = 0.0f, float py = 0.0f, float pz = 0.0f, int pl = 0){
        x = px;
        y = py;
        z = pz;
        label = pl;
...
using PointVector = vector<semicp_PointType, Eigen::aligned_allocator<semicp_PointType>>;
...
struct XYZLD_PointType
{
    double x,y,z;
    int label;
    XYZLD_PointType (double px = 0.0, double py = 0.0, double pz = 0.0, int pl = 0){
        x = px;
        y = py;
        z = pz;
        label = pl;
...
// using XYZLD_PointType = pcl::PointXYZL;
using XYZLD = vector<XYZLD_PointType, Eigen::aligned_allocator<XYZLD_PointType>>;
using XYZLF = vector<pcl::PointXYZL, Eigen::aligned_allocator<pcl::PointXYZL>>;
struct BoxPointType{
    float vertex_min[3];
    float vertex_max[3];
    BoxPointType(float x_min = 0.0f, float x_max = 0.0f, float y_min = 0.0f, float y_max = 0.0f, float z_min = 0.0f, float z_max = 0.0f){
        vertex_min[0] = x_min;
        vertex_max[0] = x_max;
        vertex_min[1] = y_min;
        vertex_max[1] = y_max;
...
class MANUAL_Q{
    private:
        int head = 0,tail = 0, counter = 0, Q_LEN = 5000;
        vector<T> q;
        bool is_empty = true;
    public:
        void pop();
...
public:
    KD_TREE(float delete_param = 0.5f, float balance_param = 0.6f , float box_length = 0.2f);
    ~KD_TREE();
    ...
    void InitializeKDTree(float delete_param = 0.5f, float balance_param = 0.7f, float box_length = 0.2f); 
    int size();
    int validnum();
...
private:
    ...
    float alpha_bal_tmp = 0.5f, alpha_del_tmp = 0.0f;
...
template class KD_TREE<pcl::PointXYZL>;
template class KD_TREE<semicp_PointType>;
// **************************************************************

// struct Operation_Logger_Type_test{
//         semicp_PointType point;
//         BoxPointType boxpoint;
//         bool tree_deleted = false;
//         bool tree_downsample_deleted = false;
//         operation_set op;
...
//     //     int size();
// };
// template class MANUAL_test<Operation_Logger_Type_test>;
...
// private:
//     int search_mutex_counter = 0;
//     MANUAL_test<Operation_Logger_Type_test> Rebuild_Logger;

// };
// template class test<semicp_PointType>;
