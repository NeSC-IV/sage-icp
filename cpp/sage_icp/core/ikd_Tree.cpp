    ...
    while (!terminated){
        ...
        if (Rebuild_Ptr != nullptr ){                    
            ...
            if (int(Rebuild_PCL_Storage.size()) > 0){
                BuildTree(&new_root_node, 0, static_cast<int>(Rebuild_PCL_Storage.size())-1, Rebuild_PCL_Storage);
...
void KD_TREE<PointType>::Build(PointVector point_cloud){
    ...
    BuildTree(&STATIC_ROOT_NODE->left_son_ptr, 0, static_cast<int>(point_cloud.size()-1), point_cloud);
...
int KD_TREE<PointType>::Add_Points(PointVector & PointToAdd, bool downsample_on){
    ...
    int tmp_counter = 0;
    for (int i=0; i<static_cast<int>(PointToAdd.size());i++){
        if (downsample_switch){
            ...
            mid_point.x = Box_of_Point.vertex_min[0] + (Box_of_Point.vertex_max[0]-Box_of_Point.vertex_min[0])/2.0f;
            mid_point.y = Box_of_Point.vertex_min[1] + (Box_of_Point.vertex_max[1]-Box_of_Point.vertex_min[1])/2.0f;
            mid_point.z = Box_of_Point.vertex_min[2] + (Box_of_Point.vertex_max[2]-Box_of_Point.vertex_min[2])/2.0f;
            ...
            for (int index = 0; index < static_cast<int>(Downsample_Storage.size()); index++){
...
void KD_TREE<PointType>::Add_Point_Boxes(vector<BoxPointType> & BoxPoints){     
    for (int i=0;i < static_cast<int>(BoxPoints.size());i++){
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
...
void KD_TREE<PointType>::Delete_Points(PointVector & PointToDel){        
    for (int i=0;i<static_cast<int>(PointToDel.size());i++){
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){               
...
void KD_TREE<PointType>::acquire_removed_points(PointVector & removed_points){
    pthread_mutex_lock(&points_deleted_rebuild_mutex_lock); 
    for (int i = 0; i < static_cast<int>(Points_deleted.size());i++){
        removed_points.push_back(Points_deleted[i]);
    }
    for (int i = 0; i <static_cast<int>(Multithread_Points_deleted.size());i++){
...
void KD_TREE<PointType>::BuildTree(KD_TREE_NODE ** root, int l, int r, PointVector & Storage){
    ...
    *root = new KD_TREE_NODE;
    ...
    for (i=1;i<3;i++) if (dim_range[i] > dim_range[div_axis]) div_axis = static_cast<uint8_t>(i);
...
void KD_TREE<PointType>::Rebuild(KD_TREE_NODE ** root){    
    ...
    if ((*root)->TreeSize >= Multi_Thread_Rebuild_Point_Num) { 
        ...
        BuildTree(root, 0, static_cast<int>(PCL_Storage.size())-1, PCL_Storage);
...
void KD_TREE<PointType>::Add_by_point(KD_TREE_NODE ** root, PointType point, bool allow_rebuild, int father_axis){     
    if (*root == nullptr){
        *root = new KD_TREE_NODE;
        InitTreeNode(*root);
        (*root)->point = point;
        (*root)->division_axis = static_cast<uint8_t>((father_axis + 1) % 3);
        Update(*root);
        return;
    }
    (*root)->working_flag = true;
...
void KD_TREE<PointType>::Search_by_radius(KD_TREE_NODE *root, PointType point, float radius, PointVector &Storage)
{
    ...
    range_center.x = (root->node_range_x[0] + root->node_range_x[1]) * 0.5f;
    range_center.y = (root->node_range_y[0] + root->node_range_y[1]) * 0.5f;
    range_center.z = (root->node_range_z[0] + root->node_range_z[1]) * 0.5f;
...
bool KD_TREE<PointType>::Criterion_Check(KD_TREE_NODE * root){
    ...
    delete_evaluation = float(root->invalid_point_num)/ float(root->TreeSize);
    balance_evaluation = float(son_ptr->TreeSize) / float(root->TreeSize-1);  
...
void KD_TREE<PointType>::Update(KD_TREE_NODE * root){
    ...
    float x_L = (root->node_range_x[1] - root->node_range_x[0]) * 0.5f;
    float y_L = (root->node_range_y[1] - root->node_range_y[0]) * 0.5f;
    float z_L = (root->node_range_z[1] - root->node_range_z[0]) * 0.5f;
    ...
    if (root == Root_Node && root->TreeSize > 3){
        KD_TREE_NODE * son_ptr = root->left_son_ptr;
        if (son_ptr == nullptr) son_ptr = root->right_son_ptr;
        float tmp_bal = float(son_ptr->TreeSize) / float(root->TreeSize-1);
        root->alpha_del = float(root->invalid_point_num)/ float(root->TreeSize);
